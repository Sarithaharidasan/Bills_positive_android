...
public class Bills_positive_scenarios extends Generic_functions {
	...
	try {
		    page_wait(20);
			click("bills_paid");
			value = driver.findElement(By.xpath(OR_reader("Object Locator", "bills_paid_validation_msg"))).isDisplayed();
			Assert.assertEquals(true,value);
			click("bills_due");
			value = driver.findElement(By.xpath(OR_reader("Object Locator", "bills_due_validation_msg"))).isDisplayed();
			Assert.assertEquals(true,value);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void bills_positive_tc_008() {
		try {
			page_wait(20);
